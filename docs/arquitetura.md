...
Mensagem não-texto chega com `"type": "audio" | "image" | …` e `"text": null` — quem decide a resposta continua sendo o n8n. Eventos de sessão usam `"type": "session.status"`, com `status` e `phoneE164` no lugar de `message`.
...
| **AI Agent** | `gemini-3.1-flash-lite`, com 3 tentativas contra sobrecarga |
| **Redis Chat Memory** | db 0, janela de **4 mensagens** (2 turnos), TTL de 1h |
| **Embeddings Google Gemini** | `gemini-embedding-2` — precisa ser **o mesmo modelo da base**, ver [base-de-faqs.md](base-de-faqs.md#a-regra-que-quebra-tudo-em-silêncio) |
...
> A conclusão que importa para o planejamento: **pergunta sem resposta aqui é falta de conteúdo na base, não corte apertado.** A correção é escrever as FAQs que faltam — é o que a tela de curadoria faz.
